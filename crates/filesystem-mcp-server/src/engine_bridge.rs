...
pub fn resolve_engine_script() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("FILESYSTEM_ENGINE_SCRIPT") {
...
pub fn invoke_ts_engine(tool: &str, arguments: Value) -> Result<CallToolResult, rmcp::ErrorData> {
    let script = resolve_engine_script().ok_or_else(|| {
        rmcp::ErrorData::invalid_request(
            format!(
                "TypeScript engine bridge unavailable. Build the package (`bun run build`) and set FILESYSTEM_ENGINE_SCRIPT to {ENGINE_INVOKE_RELATIVE}."
            ),
            None,
        )
    })?;
