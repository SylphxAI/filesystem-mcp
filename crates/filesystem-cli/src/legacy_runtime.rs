...
pub fn resolve_legacy_runtime_script() -> Option<PathBuf> {
    if let Ok(path) = std::env::var("FILESYSTEM_LEGACY_ENGINE_SCRIPT") {
...
pub fn handle_legacy_mcp_tool(
    ...
    let script = resolve_legacy_runtime_script().ok_or_else(|| ErrorEnvelope {
        ...
        message: format!(
            "Legacy MCP engine runtime is unavailable (expected {LEGACY_RUNTIME_RELATIVE})."
        ),
        next_action: "Run `bun run build` to compile the legacy engine runtime.".into(),
    })?;

    let node = std::env::var("FILESYSTEM_NODE").unwrap_or_else(|_| "node".into());
    ...
    Ok(LegacyToolSuccessEnvelope {
        status: "ok",
        engine: filesystem_core::ENGINE_NAME,
        version: filesystem_core::ENGINE_VERSION,
        tool: tool.into(),
        result,
    })
}
