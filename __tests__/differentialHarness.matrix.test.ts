...
describe('filesystem-mcp differential harness (rej-010 list-files)', () => {
	it('ships fail-closed differential entrypoint and oracle artifacts', () => {
		expect(existsSync(path.join(repoRoot, 'scripts/run-filesystem-mcp-differential.sh'))).toBe(
			true,
		)
		expect(
			existsSync(path.join(repoRoot, 'scripts/differential/filesystem-mcp-oracle.ts')),
		).toBe(true)
		expect(
			existsSync(path.join(repoRoot, 'scripts/differential/fixtures/filesystem-mcp-corpus.json')),
		).toBe(true)
		expect(
			existsSync(
				path.join(repoRoot, 'crates/filesystem-mcp-server/tests/filesystem_mcp_differential.rs'),
			),
		).toBe(true)

		const harness = readFileSync(
			path.join(repoRoot, 'scripts/run-filesystem-mcp-differential.sh'),
			'utf8',
		)
	...
	it('parity slice manifest binds list_files bounded domain', () => {
		...
		expect(slice.domains.find((domain) => domain.id === 'tool/list_files')?.boundedSlice).toBe(
			'list-files',
		)
	})
