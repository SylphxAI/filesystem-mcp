...
import { cleanupTemporaryFilesystem, createTemporaryFilesystem } from '../test-utils.js'

// Remove vi.doMock for fs/promises

// Import the core function and types
import type { LocalMcpResponse, SearchFilesDependencies } from '../../src/handlers/search-files.js'
import {
...
describe('handleSearchFiles Integration Tests', () => {
	...
	it('should find search term in multiple files with default file pattern (*)', async () => {
		...
		expect(
			(result as TestSearchResult[]).some(
				(r) => r.line === 3 && r.match === 'Search term' && r.context?.includes('Line 3: Search term here'),
			),
		).toBe(true)
		expect(
			(result as TestSearchResult[]).some(
				(r) => r.line === 2 && r.match === 'Search term' && r.context?.includes('// Search term here too'),
			),
		).toBe(true)
		expect(
			(result as TestSearchResult[]).some(
				(r) => r.line === 1 && r.match === 'Search term' && r.context?.includes('Search term in hidden file'),
			),
		).toBe(true)
	...
	it('should handle file read errors (e.g., EACCES) gracefully and continue', async () => {
		...
		mockReadFile.mockImplementation(
			async (filePath: PathLike, options?: { encoding?: string | null } | string | null): Promise<string> => {
		...
		const expectedRelativePath = path.relative(mockDependencies.PROJECT_ROOT, readablePath).replaceAll('\\', '/')
		...
		expect(errorResult?.file).toBe(path.relative(mockDependencies.PROJECT_ROOT, unreadablePath).replaceAll('\\', '/'))
