...
vi.mock('../../src/utils/path-utils', () => ({
  resolvePath: vi
    .fn()
    .mockImplementation((path) => `/project-root/${path}`)
...
describe('chmod-items handler', () => {
  ...
  beforeEach(async () => {
    ...
    // 設置默認模擬實現
    pathUtilsMock.resolvePath.mockImplementation((path: string) => `/project-root/${path}`);
  ...
  it('should handle multiple operations with mixed results', async () => {
    fsMock.chmod.mockResolvedValueOnce(undefined).mockRejectedValueOnce({ code: 'EPERM' });
