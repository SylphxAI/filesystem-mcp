import type { PathLike, WriteFileOptions } from 'node:fs' // Import PathLike type
import * as fsPromises from 'node:fs/promises'
import path from 'node:path'
import { ErrorCode, McpError } from '@modelcontextprotocol/sdk/types.js' // Re-add ErrorCode
...
describe('handleWriteContent Integration Tests', () => {
	...
	it('should handle mixed success and failure cases', async () => {
		...
		mockWriteFile.mockImplementation(async (p: PathLike, content: string | Buffer, options: WriteFileOptions) => {
			if (p.toString().endsWith('dir1')) {
	...
	it('should throw McpError for invalid top-level arguments (e.g., items not an array)', async () => {
		...
		await expect(handleWriteContentFunc(mockDependencies, invalidRequest)).rejects.toThrow(McpError)
		await expect(handleWriteContentFunc(mockDependencies, invalidRequest)).rejects.toThrow(/Invalid arguments: items/)
	})
	...
	it('should handle unexpected rejections in processSettledResults', async () => {
		...
		mockWriteFile.mockImplementation(async (p: PathLike, _content: string | Buffer, _options: WriteFileOptions) => {
