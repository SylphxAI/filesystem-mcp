...
describe('handleCopyItems Integration Tests', () => {
	...
	it('should copy a file into an existing directory', async () => {
		...
		const content = await fsPromises.readFile(path.join(tempRootDir, 'existingTargetDir/copiedFile.txt'), 'utf8')
		expect(content).toBe('Copy me!')
	})
	...
	it('should copy a directory recursively to a new location', async () => {
		...
		const content1 = await fsPromises.readFile(path.join(tempRootDir, 'copiedDir/nestedFile.txt'), 'utf8')
		expect(content1).toBe('I am nested.')
		const content2 = await fsPromises.readFile(path.join(tempRootDir, 'copiedDir/subDir/deepFile.js'), 'utf8')
	...
	it('should handle permission errors during copy', async () => {
		...
		mockCp.mockImplementation(async (src: string | URL, dest: string | URL, opts?: fs.CopyOptions) => {
			// Use string | URL
	...
	it('should handle unexpected errors during path resolution within the map', async () => {
		...
		const goodResult = result.find((r: { destination: string }) => r.destination === 'goodDest.txt')
		expect(goodResult).toBeDefined()
		expect(goodResult.success).toBe(true)

		const errorResult = result.find((r: { destination: string }) => r.destination === 'unexpected_resolve_error_dest')
