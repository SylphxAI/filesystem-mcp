...
describe('handleMoveItems Core Logic Tests', () => {
  ...
  it('should return error if source does not exist (ENOENT on access)', async () => {
    ...
    expect(mockAccess).toHaveBeenCalledWith(
      path.join(mockPathUtils.PROJECT_ROOT, 'nonexistent.txt'),
    );
  ...
  it('should handle multiple operations with mixed results', async () => {
    ...
    expect(result).toEqual([
      ...
      {
        source: 'perm-error.txt',
        destination: 'fail2.txt',
        success: false,
        error: "Permission denied moving 'perm-error.txt' to 'fail2.txt'.",
      },
    ]);
    ...
    expect(mockMkdir).toHaveBeenCalledWith(path.join(mockPathUtils.PROJECT_ROOT, 'newDir'), {
      recursive: true,
    });
  });
  ...
  it('should create destination directory if it does not exist', async () => {
    ...
    expect(result).toEqual([
      { source: 'fileToMove.txt', destination: 'newDir/movedFile.txt', success: true },
    ]);
    expect(mockMkdir).toHaveBeenCalledWith(path.join(mockPathUtils.PROJECT_ROOT, 'newDir'), {
      recursive: true,
    });
  ...
  it('validateMoveOperation should return error for invalid op', async () => {
    // Add async
