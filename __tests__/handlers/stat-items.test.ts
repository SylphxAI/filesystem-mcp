...
import { ErrorCode, McpError } from '@modelcontextprotocol/sdk/types.js' // Match source import path
import { afterEach, beforeEach, describe, expect, it, vi } from 'vitest'
import type { StatResult } from '../../src/handlers/stat-items'
import { cleanupTemporaryFilesystem, createTemporaryFilesystem } from '../test-utils.js' // Assuming a test utility exists, add .js extension
...
describe('handleStatItems Integration Tests', () => {
	...
	function assertSuccessStat(resultItem: StatResult, expectedPath: string, expectedProps: ExpectedStatProps): void {
		expect(resultItem.stats, `Expected stats object for successful path '${expectedPath}'`).toBeDefined()
		...
		if (expectedProps.size !== undefined) {
			expect(resultItem.stats?.size, `Expected size=${expectedProps.size} for path '${expectedPath}'`).toBe(
				expectedProps.size,
			)
		}
	...
	function assertErrorStat(resultItem: StatResult, expectedPath: string, expectedProps: ExpectedStatProps): void {
		expect(resultItem.stats, `Expected no stats object for error path '${expectedPath}'`).toBeUndefined()
		...
		if (expectedProps.error) {
			if (expectedProps.error instanceof RegExp) {
				expect(resultItem.error, `Error message for path '${expectedPath}' did not match regex`).toMatch(
					expectedProps.error,
				)
			} else {
				expect(resultItem.error, `Error message for path '${expectedPath}' did not match string`).toBe(
					expectedProps.error,
				)
			}
		}
	...
	function assertStatResult(results: StatResult[], expectedPath: string, expectedProps: ExpectedStatProps): void {
		...
		expect(resultItem.status, `Expected status '${expectedProps.status}' for path '${expectedPath}'`).toBe(
			expectedProps.status,
		)
