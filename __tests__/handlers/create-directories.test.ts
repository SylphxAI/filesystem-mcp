...
import { afterEach, beforeEach, describe, expect, it, type Mock, vi } from 'vitest' // Added Mock type
...
import {
	type CreateDirsDeps, // Import deps type
...
describe('handleCreateDirectories Integration Tests', () => {
	...
	it('should reject requests with empty paths array based on Zod schema', async () => {
		const request = { paths: [] }
		await expect(handleCreateDirectoriesInternal(request, mockDependencies)).rejects.toThrow(McpError)
	...
	it.skip('should handle unexpected errors during path resolution within the map', async () => {
		...
		const badResult = result.find((r: CreateDirResultForTest) => r.path === 'unexpected_resolve_error')
	...
	it('should throw McpError for invalid top-level arguments (e.g., paths not an array)', async () => {
		...
		await expect(handleCreateDirectoriesInternal(invalidRequest, mockDependencies)).rejects.toThrow(McpError)
